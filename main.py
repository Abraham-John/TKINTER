...
class ZodiacDatabase:
    ...
    def update_entry(self, entry_id, zodiac_sign, horoscope, lucky_number, lucky_color, compatibility):
        try:
            with self.mydb.cursor() as cursor:
                sql = "UPDATE horoscope SET zodiac_sign=%s, horoscope=%s, lucky_number=%s, lucky_color=%s, compatibility=%s WHERE id=%s"
                val = (zodiac_sign, horoscope, lucky_number, lucky_color, compatibility, entry_id)
                cursor.execute(sql, val)
            self.mydb.commit()
...
class ZodiacApp:
    ...
    def delete_latest_entries(self):
        num_entries = 1
        database = ZodiacDatabase(host="localhost", user="root", password="valt", database="user_zodiac")
        database.delete_latest_entries(num_entries)
    ...
    def get_entry_data(self, entry_id):
        database = ZodiacDatabase(host="localhost", user="root", password="valt", database="user_zodiac")
        try:
            with database.mydb.cursor() as cursor:
    ...
    def update_entry(self, entry_id, zodiac_sign, horoscope, lucky_number, lucky_color, compatibility):
        database = ZodiacDatabase(host="localhost", user="root", password="valt", database="user_zodiac")
        database.update_entry(entry_id, zodiac_sign, horoscope, lucky_number, lucky_color, compatibility)
